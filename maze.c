...
float expectedLeftGap = 8.5;
float expectedRightGap = 13.7;
...
void showStep()
{
    for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            ...
            {
                displayStringAt(LCD_HEIGHT - i * CHAR_HEIGHT, j * CHAR_WIDTH, "%c", getDirectionSymbol(pathMap[i][j].direction));
            }
            else
            {
                displayStringAt(LCD_HEIGHT - i * CHAR_HEIGHT, j * CHAR_WIDTH, " ");
            }
        }
    }
}
...
void showSearchStep()
{
		for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            ...
            {
                displayStringAt(i * CHAR_WIDTH, LCD_HEIGHT - j * CHAR_HEIGHT, " %d", searchMap[i][j].carWentedDirection);
            }
            else
            {
                displayStringAt(i * CHAR_WIDTH, LCD_HEIGHT - j * CHAR_HEIGHT, "%d", -1);
            }
        }
    }
...
{
    int turnToWhere = turn + 1 - currentUltraServo;
    for (int i = 0; i < absolute(turnToWhere); i++)
    {
        if (turnToWhere > 0)
        {
            turnUltraServo(RIGHT_ULTRA_SERVO);
        }
        else if (turnToWhere < 0)
        {
    ...
    for (int i = 0; i < abs(turnToWhere); i++)
    {
        if (turnToWhere > 0)
        {
            turnUltraServo(LEFT_ULTRA_SERVO);
